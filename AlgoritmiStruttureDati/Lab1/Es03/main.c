...
void carica(struct corsa listaCorse[]);
...
int main()
{
    ...
    printf("stampa <video/file>->STAMPA Log\n");
    ...
    printf("cerca <stazione> ->RICERCA Tratte per Stazione Partenza\n");
    ...
    while(codComando != r_fine)
    {
            ...
            menuParola(listaCorse, n, codComando);
...
comando_e leggiComando(){
    ...
    ///Primo comando
    c=r_stampa;
    ///Finch� non troviamo il comando nella tabella
...
int confronto(struct corsa listaCorse[], char tipo[], int j){
    ...
    if(strcmp(tipo, "data")==0)
    {
        ...
        ///Se la data j-esima � maggiore della j-esima +1
...
int sostituisci(struct corsa listaCorse[],  int j)
{
        ...
        strcpy(temp.codice , listaCorse[j].codice );
        strcpy(temp.partenza , listaCorse[j].partenza );
        strcpy(temp.destinazione , listaCorse[j].destinazione );
        strcpy(temp.data , listaCorse[j].data );
        strcpy(temp.oraP , listaCorse[j].oraP );
        strcpy(temp.oraA , listaCorse[j].oraA );
        temp.ritardo =listaCorse[j].ritardo;

        ///A[j]=A[j+1];

        strcpy(listaCorse[j].codice, listaCorse[j+1].codice);
        strcpy(listaCorse[j].partenza, listaCorse[j+1].partenza);
        strcpy(listaCorse[j].destinazione, listaCorse[j+1].destinazione);
        strcpy(listaCorse[j].data, listaCorse[j+1].data);
        strcpy(listaCorse[j].oraP, listaCorse[j+1].oraP);
        strcpy(listaCorse[j].oraA, listaCorse[j+1].oraA);
        listaCorse[j].ritardo=listaCorse[j+1].ritardo;


        ///A[j+1]= temp

        strcpy(listaCorse[j+1].codice, temp.codice);
        strcpy(listaCorse[j+1].partenza, temp.partenza);
        strcpy(listaCorse[j+1].destinazione, temp.destinazione);
        strcpy(listaCorse[j+1].data, temp.data);
        strcpy(listaCorse[j+1].oraP, temp.oraP);
        strcpy(listaCorse[j+1].oraA, temp.oraA);
        listaCorse[j+1].ritardo= temp.ritardo;
...
int cerca(struct corsa listaCorse[], int n, char s[], int vet[])
{
        ...
        for(i=0;i<n;i++)
        {
            if(strstr(listaCorse[i].partenza, s) != NULL)///Puntatore ritornato non � NULL
            {
                ///Cos� facendo prendiamo il PREFISSO
